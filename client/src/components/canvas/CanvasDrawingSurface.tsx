  ...
  return (
    <>
      {strokes.map((stroke) => {
        ...
        const pts = isDragged
          ? stroke.points.map((p) => ({
              x: p.x + selectionDragOffset.dx,
              y: p.y + selectionDragOffset.dy,
            }))
          : stroke.points;
...
export default function CanvasDrawingSurface({
  ...
  const viewBoxStr =
    vbW !== undefined && vbH !== undefined ? `${pan.x} ${pan.y} ${vbW} ${vbH}` : undefined;
  ...
  return (
    <>
      ...
      <div className="canvas-workspace">
        ...
        >
          ...
          {Object.entries(layers).map(([uid, layer]) => {
            if (soloUserId ? uid !== soloUserId : !layer.visible) {
              return null;
            }
            return (
              <StrokeLayer
          ...
          {activeUsers.map((u) => {
            ...
            for (const layer of Object.values(layers)) {
              if (soloUserId ? layer.userId !== soloUserId : !layer.visible) {
                continue;
              }
              for (const s of layer.strokes) {
                if (selSet.has(s.id)) {
                  strokesToOutline.push(s);
                }
              }
            }
            if (!strokesToOutline.length) {
              return null;
            }
            return (
              <g
      ...
      <div className="stats-bar">
        ...
        {zoom !== 1 && (
          <div className="stat stat--zoom">
            {Math.round(zoom * 100)}% <span className="stat-hint">(Ctrl+scroll, Ctrl+0)</span>
          </div>
        )}
        {selectedCount > 0 && (
