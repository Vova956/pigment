      ...
      <div className="panel-section">
        ...
        {Object.values(layers).some((l) => (l.userId ?? '') === userId) && (
          <div
            ...
            style={{
              cursor: 'pointer',
              borderLeft:
                activeLayerId === ALL_LAYERS_SENTINEL
                  ? '2.5px solid #0284c7'
          ...
          >
            ...
            <div className="layer-info">
              <div className="layer-name">
                All my layers
                {activeLayerId === ALL_LAYERS_SENTINEL && (
                  <span className="user-role" style={{ marginLeft: 6 }}>
                    Active
                  </span>
                )}
              </div>
        ...
        {Object.entries(layers).map(([lid, layer]) => {
          ...
          const pickLayer = (e: React.MouseEvent) => {
            ...
            if (el.closest('.layer-actions') || el.closest('.layer-comments')) {
              return;
            }
            ...
            >
              <div className={`layer-item${isSolo ? ' active' : ''}`}>
                ...
                <div className="layer-actions" onClick={(e) => e.stopPropagation()}>
                  ...
                  >
                    ...
                    {comments.length > 0 && (
                      <span className="layer-comment-badge">{comments.length}</span>
                    )}
                  </button>
                </div>
              </div>
              ...
              {commentsOpen && (
                <div className="layer-comments" onClick={(e) => e.stopPropagation()}>
                  {comments.length === 0 && (
                    <div className="layer-comment-empty">No comments yet.</div>
                  )}
                  {comments.map((c) => (
                    <div key={c.id} className="layer-comment">
                      <div
                        className="layer-comment-avatar"
                        style={{ background: UserColorService.getColor(c.userId) }}
                      >
                  ...
                  <div className="layer-comment-input-row">
                    <input
                      ...
                      onChange={(e) =>
                        setCommentInputs((prev) => ({ ...prev, [uid]: e.target.value }))
                      }
            ...
            .map((ev) => (
              <div key={ev.id} className="activity-item">
                <div
                  ...
                  style={{ background: UserColorService.getColor(ev.name) }}
                >
                  {ev.avatar}
                </div>
                <div>
                  <div className="activity-text">
                    <strong>{ev.name}</strong> {ev.action}
                  </div>
            ...
            chatMessages.map((msg) => (
              <div key={msg.id} className="chat-msg">
                <div
                  ...
                  style={{ background: UserColorService.getColor(msg.userId) }}
                >
