...
// Wire payload is dynamic JSON from the websocket — we validate shape-by-shape
// in the switch below. Typed permissively so each case can assert the fields
// it needs without ceremony.
interface IncomingMessage {
  type: string;
  stroke: Stroke;
  layerId?: string;
  strokeIds?: string[];
  strokes?: Array<{ id: string; points: Point[] }>;
  image: CanvasImage;
  imageId?: string;
  x: number;
  y: number;
...
export async function applyMessage(sessionId: string, msg: IncomingMessage): Promise<boolean> {
  ...
  switch (msg.type) {
    case 'stroke': {
      ...
      if (existing.strokes.some((s) => s.id === msg.stroke.id)) {
        return false;
      }
    ...
    case 'image':
      if (snap.images.some((i) => i.id === msg.image.id)) {
        return false;
      }
    ...
    case 'text':
      if (snap.texts.some((t) => t.id === msg.canvasText.id)) {
        return false;
      }
